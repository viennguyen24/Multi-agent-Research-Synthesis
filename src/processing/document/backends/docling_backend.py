...
class DoclingBackend(OCRBackend):
    ...
    def extract(self, source_path: str) -> ExtractionResult:
        ...
        chunks = self._extract_text_chunks(document)
        images, tables, equations = self._extract_multimodal_artifacts(document, markdown)
    ...
    def _extract_text_chunks(self, document: DoclingDocument) -> list[ExtractedChunk]:
        """
        Performs semantic chunking using HybridChunker.
        Collects page numbers and docling item labels per chunk.
        """
        ...
        for idx, chunk in enumerate(chunker.chunk(dl_doc=document)):
            ...
            extracted_chunks.append(
                ExtractedChunk(
                    ...
                    doc_item_labels=sorted(set(labels)),
                )
            )
        return extracted_chunks

    def _extract_multimodal_artifacts(
        self, document: DoclingDocument, markdown: str
        ...
        for idx, item in enumerate(document.pictures, start=1):
            res = self._extract_image(item, f"img_{idx:03d}", document)
            if res is not None:
                images.append(res)
        ...
        for idx, item in enumerate(document.tables, start=1):
            res = self._extract_table(item, f"tbl_{idx:03d}", document)
            if res is not None:
                tables.append(res)

        equations = self._extract_equations(document, markdown)
